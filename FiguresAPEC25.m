...
N = linspace(10, 40, 30)';  % [num_N x 1] number of turns (column vector)
...
nl = linspace(4,10,6);  % [1 x num_nl] number of PCB layers (row vector)
...
% COLOR MAP
color1 = hex2rgb(char(colorsDARK(5)));  % Start color
color2 = hex2rgb(char(colorsPAST(1)));  % Middle color
color3 = hex2rgb(char(colorsNORM(2)));  % End color
...
subplot(2,2,1);
contourf(N_plot, nl_plot, fp_cm2_plot, 20, 'LineStyle', 'none'); hold on;
[C,h] = contour(N_plot, nl_plot, fp_cm2_plot, 10, 'LineColor', 'k');
ht = clabel(C, h, 'FontSize', 8, 'Color', 'k'); hold off;
colormap(cmap_dark); c = colorbar; c.Label.String = 'Footprint (cm²)'; caxis([50, 180]);  % Set color axis limits
xlabel('Number of Turns'); ylabel('Number of Layers');
subplot(2,2,2);
contourf(N_plot, nl_plot, P_total_non_plot, 20, 'LineStyle', 'none'); hold on;
[C,h] = contour(N_plot, nl_plot, P_total_non_plot, 10, 'LineColor', 'k');
ht = clabel(C, h, 'FontSize', 8, 'Color', 'k'); hold off;
colormap(cmap_dark); c = colorbar; c.Label.String = 'Total Loss (W)'; caxis([0.5,10]);  % Set color axis limits
xlabel('Number of Turns'); ylabel('Number of Layers');
set(findall(fig, '-property', 'FontName'), 'FontName', 'Cambria', 'Fontsize', 12);

subplot(2,2,3);
contourf(N_plot, nl_plot, P_total_1in_plot, 20, 'LineStyle', 'none'); hold on;
[C,h] = contour(N_plot, nl_plot, P_total_1in_plot, 10, 'LineColor', 'k');
ht = clabel(C, h, 'FontSize', 8, 'Color', 'k'); hold off;
colormap(cmap_dark); c = colorbar; c.Label.String = 'Total Loss (W)'; caxis([0.5,10]);  % Set color axis limits
xlabel('Number of Turns'); ylabel('Number of Layers');
set(findall(fig, '-property', 'FontName'), 'FontName', 'Cambria', 'Fontsize', 12);
...
subplot(2,2,4);
contourf(N_plot, nl_plot, P_total_2in_plot, 20, 'LineStyle', 'none'); hold on;
[C,h] = contour(N_plot, nl_plot, P_total_2in_plot, 10, 'LineColor', 'k');
ht = clabel(C, h, 'FontSize', 8, 'Color', 'k'); hold off;
colormap(cmap_dark); c = colorbar; c.Label.String = 'Total Loss (W)'; caxis([0.5,10]);  % Set color axis limits
xlabel('Number of Turns'); ylabel('Number of Layers');
set(findall(fig, '-property', 'FontName'), 'FontName', 'Cambria', 'Fontsize', 12);
...
[C,h] = contour(N_plot, nl_plot, fp_cm2_plot, 10, 'LineColor', 'k');
ht = clabel(C, h, 'FontSize', 8, 'Color', 'k');
...
if plotHioki == true
    ...
    h0 = plot(f_v3, L_v3*1e3, '-', 'color', colorsPAST(5), 'LineWidth', 1.2);
    xlabel('Frequency (Hz)'); ylabel('Inductance (uH)');
    xlim([250e3, 500e3]); ylim([-2.2, 2.6]);
    grid on; ax = gca;
    ax.GridLineStyle = ':'; ax.GridColor = 'k'; ax.GridAlpha = 1; box on;
    ax.XScale = "log";
    set(findall(fig, '-property', 'FontName'), 'FontName', 'Cambria', 'Fontsize', 12);
    %figName = "hioki_oc.pdf"; exportgraphics(fig, figName, 'BackgroundColor', 'none', 'ContentType', 'vector');
    %figName = "hioki_oc.png"; exportgraphics(fig, figName, 'BackgroundColor', 'white', 'Resolution', 600);
end
